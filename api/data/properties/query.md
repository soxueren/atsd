...
| keyExpression | string | Expression for matching properties with specified keys or tags.<br>Example: `file_system LIKE '/u*'` or `tags.fs_type == 'ext4'`.<br>Names are case-insensitive whereas values are case-sensitive. <br>`file_system LIKE '/u*'` will match `/user123` but not `/User123`.<br>Use `lower()` function to ignore case, for example `lower(file_system) LIKE '/u*'`|
