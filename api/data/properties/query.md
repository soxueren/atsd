...
* `key` and `keyTagExpression` fields are mutually exclusive, only one of them can be specified in the query object.
* `key`, `keyTagExpression`, and `exactMatch` fields are ignored when querying `$entity_tags` type.
