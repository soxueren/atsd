...
| `property_compare_except([S key], [S prevValue])` |   Same as `property_compare_except([S key])` with the list of previous values that are excluded from the difference map. |
