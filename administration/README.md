# Administration
- [Deployment](deployment.md#deployment)
- [Setup Email Client](setting-up-email-client.md#setting-up-the-email-client)
- [User Authentication](user-authentication.md#user-authentication)
  - [Authentication Mechanisms](user-authentication.md#authentication-mechanisms)
  - [User Account Types](user-authentication.md#user-account-types)
  - [Password Requirements](user-authentication.md#password-requirements)
  - [Built-In Account](user-authentication.md#built-in-account)
  - [Changing the Password](user-authentication.md#changing-the-password)
  - [Resetting Password](user-authentication.md#resetting-password)
  - [Anonymous API Access](user-authentication.md#anonymous-api-access)
  - [HTTP Basic Authorization Examples](user-authentication.md#http-basic-authorization-examples)
- [User Authorization](user-authorization.md#user-authorization)
  - [Role Based Access Control](user-authorization.md#role-based-access-control)
    - [Available API roles](user-authorization.md#available-api-roles)
    - [Available User Interface roles](user-authorization.md#available-user-interface-roles)
  - [Entity Permissions](user-authorization.md#entity-permissions)
    - [All Entities Permissions](user-authorization.md#all-entities-permissions)
    - [Inserting Data for New Entities](user-authorization.md#inserting-data-for-new-entities)
    - [Wildcard Requests](user-authorization.md#wildcard-requests)
  - [Implementation Notes](user-authorization.md#implementation-notes)
- [Restarting](restarting.md#restarting)
  - [Script Directory](restarting.md#script-directory)
  - [Scripts](restarting.md#scripts)
  - [Examples](restarting.md#examples)
  - [Docker Container](restarting.md#docker-container)
  - [Processes](restarting.md#processes)
  - [Safe Restart](restarting.md#safe-restart)
- [Updating](update.md#update)
  - [Check Revision](update.md#check-revision)
  - [Update Command](update.md#update-command)
  - [Validation](update.md#validation)
  - [Restart](update.md#restart)
- [Uninstalling](uninstalling.md#uninstalling)
  - [Debian Package](uninstalling.md#debian-package)
  - [RPM Package](uninstalling.md#rpm-package)
- [Migrate ATSD to JAVA 7](migrate-to-java7.md#migrate-atsd-to-java-7)
  - [Overview](migrate-to-java7.md#overview)
  - [Install openjdk-7-jdk](migrate-to-java7.md#install-openjdk-7-jdk)
  - [Stop ATSD](migrate-to-java7.md#stop-atsd)
  - [Replace JAVA_HOME](migrate-to-java7.md#replace-java_home)
  - [Reload Environment Variables](migrate-to-java7.md#reload-environment-variables)
  - [Start ATSD](migrate-to-java7.md#start-atsd)
- [Network Settings](networking-settings.md#network-settings)
  - [Increase the Maximum Receiving Buffer on the Operating System](networking-settings.md#increase-the-maximum-receiving-buffer-on-the-operating-system)
  - [Increase the UDP Buffer in ATSD](networking-settings.md#increase-the-udp-buffer-in-atsd)
- [Enabling Swap Space](enabling-swap-space.md#enabling-swap-space)
- [Replication](replication.md#replication)
  - [Requirements](replication.md#requirements)
  - [Installation](replication.md#installation)
  - [Enabling Replication](replication.md#enabling-replication-for-new-tables)
  - [Verifying Replication](replication.md#verifying-replication)
    - [Option 1](replication.md#option-1)
    - [Option 2](replication.md#option-2)
- [Allocating Memory to components](allocating-memory.md#allocating-memory-to-components)
  - [Change maximum ATSD process memory](allocating-memory.md#change-maximum-atsd-process-memory)
  - [Change maximum HBase process memory](allocating-memory.md#change-maximum-hbase-process-memory)
  - [Change maximum HDFS process memory](allocating-memory.md#change-maximum-hdfs-process-memory)
  - [Restart all components](allocating-memory.md#restart-all-components)
  - [Verify that -Xmx parameter is set accordingly](allocating-memory.md#verify-that--xmx-parameter-is-set-accordingly)
- [Changing Data Directory](changing-data-directory.md#changing-the-directory-where-data-is-stored)
- [Editing Configuration Files](editing-configuration-files.md#editing-configuration-files)
- [Logging](logging.md#logging)
  - [Reloading Log Settings](logging.md#reloading-log-settings)
  - [Modifying command.log](logging.md#modifying-commandlog)
- [Metric Persistence Filter](metric-persistence-filter.md#metric-persistence-filter)
- [Entity Lookup](entity-lookup.md#entity-lookup)
- [Restoring a corrupted zookeeper](corrupted-zookeeper.md#restoring-a-corrupted-zookeeper)
- [Compaction](compaction.md#compaction)
- [Compaction Test](compaction-test.md#compaction-test)
- [Monitoring](monitoring.md#monitoring)
  - [Collected Metrics](monitoring.md#collected-metrics)
