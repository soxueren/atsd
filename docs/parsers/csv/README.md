...
|  Replace Entities  |  Replace entity names in the input file with their aliases from the selected [Replacement Table](/docs/administration/entity-lookup.md).<br>For example, if the Replacement Table contains a mapping `103323213=sensor001` and the entity of the CSV file is named `103323213`, then it will be saved in ATSD as `sensor001`.  |
|  Process Events  |  Process incoming data in the [Rule Engine](/docs/rule-engine) in addition to storing it in the database.  |
