...
| [3719](#issue-3719) | sql         | Feature | Optimized [windowing](/docs/api/sql#last_time-syntax) queries by narrowing the requested timespan based on minimum last insert date. |
...
| [3555](#issue-3555) | sql         | Feature | Implemented [`LOOKUP`](/docs/api/sql#lookup) function to translate the key into a value using the specified replacement table. |
| [3421](#issue-3421) | sql         | Feature | Implemented the `searched case` variant of the [`CASE`](/docs/api/sql#case) expression. |
...
We added an optimization to narrow the start date in [windowing](/docs/api/sql#last_time-syntax) queries which is now determined as the minimum (last insert date) for all series.
...
One of the use cases is to display the most recent time in windowing queries where the [last_time](/docs/api/sql#last_time-syntax) function can be utilized to select data for a sliding interval, such as the most recent 4 weeks for each series in the example below.
...
The sequence of period interpolation and period filtering with the [HAVING](/docs/api/sql#having-filter) clause was modified.
Now, the `HAVING` filter is applied after [PERIOD interpolation](/docs/api/sql#interpolation) whereas before it was the opposite.
...
[JOINs](/docs/api/sql#joins) in ATSD perform merging of rows on time, entity, and series tags anyway. Prior to this change, the tag filter
...
Implemented the special `SELECT 1` query, which is typically used to [test connectivity](/docs/api/sql#validation-query) and validate open
...
Implemented the [LOOKUP](/docs/api/sql#lookup) function, which translates the key into a value using the specified replacement table.
...
Implemented the `searched case` variant of the [CASE](/docs/api/sql#case) expression.
...
Refer to [examples](/docs/api/sql/examples/case.md) for additional information.
